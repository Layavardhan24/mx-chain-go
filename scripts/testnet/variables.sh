...
# anyway.
export TESTNETDIR="$HOME/MultiversX/Local-Devnet"
...
# Must be 0 to allow resuming from existing DBs (chain ID must match genesis in DB).
...
# Calculate validator counts including hysteresis
shard_val_count=$(awk -v n="$SHARD_VALIDATORCOUNT" -v h="$HYSTERESIS" 'BEGIN { print int(n * (1 + h) + 0.999999) }')
meta_val_count=$(awk -v n="$META_VALIDATORCOUNT" -v h="$HYSTERESIS" 'BEGIN { print int(n * (1 + h) + 0.999999) }')
let "total_node_count = $shard_val_count * $SHARDCOUNT + $meta_val_count + $TOTAL_OBSERVERCOUNT"
