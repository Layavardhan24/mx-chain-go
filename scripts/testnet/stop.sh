...
then
  # close every multiversx terminal window (both start.sh layout and per-group scripts)
  tmux kill-session -t "multiversx-tools" 2>/dev/null
  tmux kill-session -t "multiversx-nodes" 2>/dev/null
  tmux kill-session -t "multiversx-seednode" 2>/dev/null
  tmux kill-session -t "multiversx-proxy" 2>/dev/null
fi
