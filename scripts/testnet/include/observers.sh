...
assembleCommand_startObserverNode() {
  ...
  local nodeCommand="./node \
        ...
        $KEYS_FLAGS \
        -working-directory $WORKING_DIR -config ./config/config_observer.toml --config-external ./config/external_observer.toml $EXTRA_OBSERVERS_FLAGS --display-name mx-chain-devnet-observer-$OBSERVER_INDEX"
