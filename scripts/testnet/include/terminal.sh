...
setTerminalSession() {
  ...
  then
    ...
    then
      ...
      # session already exists: adopt it and continue pane numbering after
      # the panes that are already open, otherwise the counter stays empty
      # and runCommandInTerminal fails with "integer expression expected"
      CURRENT_TMUX_SESSION=$session_name
      local existing_panes=$(tmux list-panes -t "$session_name:0" 2>/dev/null | wc -l)
      if [ -z "$existing_panes" ] || [ "$existing_panes" -lt 1 ]; then
        existing_panes=0
      fi
      TMUX_SESSION_PANES[$session_name]=$((existing_panes + 1))
    fi
  fi
}
