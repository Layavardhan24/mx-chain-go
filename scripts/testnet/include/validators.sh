...
iterateOverValidators() {
  ...
  local shard_val_count=$(awk -v n="$SHARD_VALIDATORCOUNT" -v h="$HYSTERESIS" 'BEGIN { print int(n * (1 + h) + 0.999999) }')
  local meta_val_count=$(awk -v n="$META_VALIDATORCOUNT" -v h="$HYSTERESIS" 'BEGIN { print int(n * (1 + h) + 0.999999) }')

  # Iterate over Shard Validators
  (( max_shard_id=$SHARDCOUNT - 1 ))
  for SHARD in $(seq 0 1 $max_shard_id); do
    for _ in $(seq $shard_val_count); do
  ...
  SHARD="metachain"
  for _ in $(seq $meta_val_count); do
    if [ $VALIDATOR_INDEX -ne $SKIP_VALIDATOR_IDX ]; then
      $callback $SHARD $VALIDATOR_INDEX
      sleep 0.5
    fi
...
stopSingleValidator() {
  ...
  if [ "$NODE_WATCHER" == "1" ]; then
    ...
    if [ -f "$WORKING_DIR/watcher.pid" ]; then
      local w_pid=$(cat "$WORKING_DIR/watcher.pid")
      kill -9 $w_pid 2>/dev/null
      rm -f "$WORKING_DIR/watcher.pid"
    fi
  fi
...
assembleCommand_startValidatorNode() {
  ...
  local node_command="./node \
        -port $PORT --profile-mode -log-save -log-level $LOGLEVEL --log-correlation --use-health-service -rest-api-interface localhost:$RESTAPIPORT \
        -sk-index $KEY_INDEX \
        -working-directory $WORKING_DIR -config ./config/config_validator.toml --display-name mx-chain-devnet-validator-$VALIDATOR_INDEX"
