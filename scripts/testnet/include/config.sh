...
updateNodeConfig() {
  ...
  updateJSONValue nodesSetup_edit.json "minTransactionVersion" "1"
	...
	if [ $ALWAYS_NEW_CHAINID -eq 1 ]; then
		updateTOMLValue config_validator.toml "ChainID" "\"L"\"
		updateTOMLValue config_observer.toml "ChainID" "\"L"\"
	fi
  ...
  # Create external_observer.toml with outport host driver enabled
  cp external.toml external_observer.toml
  sed -i '/\[\[HostDriversConfig\]\]/,/Enabled/ s/Enabled = false/Enabled = true/' external_observer.toml
...
updateTxGenConfig() {
  ...
  updateTOMLValue config_edit.toml "ProxyServerURL" "\"http://0.0.0.0:$PORT_PROXY\""
...
generateProxyObserverList() {
  ...
  for SHARD in $(seq 0 1 $max_shard_id); do
    for _ in $(seq $SHARD_OBSERVERCOUNT); do
      ...
      echo "   Address = \"http://0.0.0.0:$PORT\"" >> config_edit.toml
      echo ""$'\n' >> config_edit.toml
